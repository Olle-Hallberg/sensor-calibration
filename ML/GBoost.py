...
search_results_df.to_csv("hyperparameter_results_gradient_boosting.csv", index=False)
...
# #finalmodel
best_model = build_gbr_model(best_params)
...
test_results.to_csv(
    f"predictions_test_{target_column}_gradient_boosting.csv",
    index=False
)
...
all_results.to_csv(
    f"predictions_all_{target_column}_gradient_boosting.csv",
    index=False
)
...
plt.title(f"Temperature Compensation (Gradient Boosting)\nR² = {r2:.3f}")

plt.legend(loc="upper right")

plt.tight_layout()
plt.savefig(f"time_series_all_{target_column}_gradient_boosting.png", dpi=300)
plt.show()