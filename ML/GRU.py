...
search_results_df.to_csv("hyperparameter_results_gru.csv", index=False)
...
test_results.to_csv(f"predictions_test_gru_{target_column}.csv", index=False)
...
all_results.to_csv(f"predictions_all_gru_{target_column}.csv", index=False)
...
ax1.set_title(f"Temperature Compensation - GRU\nR² = {r2:.3f}")
...
plt.tight_layout()
plt.savefig(f"time_series_all_gru_{target_column}.png", dpi=300)
plt.show()