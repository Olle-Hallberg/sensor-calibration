...
def build_adaboost_model(params):
    ...
    model = AdaBoostRegressor(
        estimator=base_tree,  # use base_estimator=base_tree if older sklearn
...
param_grid = {
    ...
    # loss -> loss function for boosting
    # estimator__max_depth -> tree depth
    # estimator__min_samples_split -> minimum samples to split
    # estimator__min_samples_leaf -> minimum samples in leaf
...
print("Starting AdaBoost hyperparameter search...\n")
...
for run_number, params in enumerate(all_param_combinations, start=1):
    ...
    for train_idx, val_idx in cv.split(X_train):
        ...
        model = build_adaboost_model(params)
    ...
    search_results.append({
        ...
        "learning_rate": params["learning_rate"],
        "loss": params["loss"],
        "estimator__max_depth": params["estimator__max_depth"],
        "estimator__min_samples_split": params["estimator__min_samples_split"],
        "estimator__min_samples_leaf": params["estimator__min_samples_leaf"],
...
search_results_df.to_csv("hyperparameter_results_adaboost.csv", index=False)
...
# #finalmodel
best_model = build_adaboost_model(best_params)
...
test_results.to_csv(f"predictions_test_{target_column}_adaboost.csv", index=False)
...
all_results.to_csv(f"predictions_all_{target_column}_adaboost.csv", index=False)
...
plt.title(f"Temperature Compensation (AdaBoost)\nR² = {r2:.3f}")

plt.legend(loc="upper right")

plt.tight_layout()
plt.savefig(f"time_series_all_{target_column}_adaboost.png", dpi=300)
plt.show()